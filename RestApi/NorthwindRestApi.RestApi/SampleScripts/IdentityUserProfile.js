...
var uri = 'api/ıdentityuserprofile';
...
function updateitem() {
    $.ajax({
        type: 'Put',
        url: window.getBaseUrl() + uri + "/" + $('#UserId').val(),
        data: getformvalues(),
        dataType: "json"
    })
   .done(function ()
...
function formatItem(item) {
    ...
    row += "<td><a href='#' onclick='showedit(\"" + item.UserId + "\", false)'>edit</a>";
    row += "&nbsp;<a href='#' onclick='confirmdelete(\"" + item.UserId + "\")'>delete</a></td>";
    row += "</tr>";
    return row;
}
