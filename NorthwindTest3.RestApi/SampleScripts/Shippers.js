...
function updateitem() {
    $.ajax({
        type: 'Put',
        url: window.getBaseUrl() + uri + "/" + $('#ShipperID').val(),
        data: getformvalues(),
        dataType: "json"
    })
   .done(function ()
...
function formatItem(item) {
    ...
    row += "<td><a href='#' onclick='showedit(\"" + item.ShipperID + "\", false)'>edit</a>";
    row += "&nbsp;<a href='#' onclick='confirmdelete(\"" + item.ShipperID + "\")'>delete</a></td>";
    row += "</tr>";
    return row;
}
...
function showedit(id, isnew) {
    ...
    if (isnew)
    {
        $("#ShipperID").removeAttr('readonly');
        $("#formcaption").html("Add new item");
        $("#hiddenid").val(-1);
        $("#updatebtn").html("Add");
    }
    else {
        $("#ShipperID").attr('readonly');
