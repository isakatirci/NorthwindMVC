...
{
	public partial class SalesByCategoryController : ApiController
	{
		protected ISalesByCategoryRepository _iSalesByCategoryRepository;

            // GET: api/SalesByCategory
        public async Task<IHttpActionResult> GetSalesByCategory(int page = 0, int pageSize = 5)
        {
            int totalCount = 0;

            IList<SalesByCategoryVm> listVm = new List<SalesByCategoryVm>();
            var result = Task.Factory.StartNew(() => {

                ICriteria criteria = new Criteria<BOSalesByCategory>(SalesByCategoryRepository);
                totalCount = criteria.Count();

                IList<BOSalesByCategory> listBOs = criteria
                ...
                foreach (var bo in listBOs)
                    listVm.Add(new SalesByCategoryVm(bo));
                return listVm;
            });
            await result;
        ...
        {
            get { return _iSalesByCategoryRepository ?? (_iSalesByCategoryRepository = RF.New().SalesByCategoryRepository); }
            set { _iSalesByCategoryRepository = value; }
        }
