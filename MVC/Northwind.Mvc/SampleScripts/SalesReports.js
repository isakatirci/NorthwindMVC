...
function updateitem() {
    $.ajax({
        type: 'Put',
        url: window.getBaseUrl() + uri + "/" + $('#GroupBy').val(),
        data: getformvalues(),
        dataType: "json"
    })
   .done(function ()
...
function formatItem(item) {
    ...
    row += "<td><a href='#' onclick='showedit(\"" + item.GroupBy + "\", false)'>edit</a>";
    row += "&nbsp;<a href='#' onclick='confirmdelete(\"" + item.GroupBy + "\")'>delete</a></td>";
    row += "</tr>";
    return row;
}
...
function showedit(id, isnew) {
    ...
    if (isnew)
    {
        $("#GroupBy").removeAttr('readonly');
        $("#formcaption").html("Add new item");
        $("#hiddenid").val(-1);
        $("#updatebtn").html("Add");
    }
    else {
        $("#GroupBy").attr('readonly');
