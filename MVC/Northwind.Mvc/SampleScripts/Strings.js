...
function updateitem() {
    $.ajax({
        type: 'Put',
        url: window.getBaseUrl() + uri + "/" + $('#Stringİd').val(),
        data: getformvalues(),
        dataType: "json"
    })
   .done(function ()
...
function formatItem(item) {
    ...
    row += "<td><a href='#' onclick='showedit(\"" + item.Stringİd + "\", false)'>edit</a>";
    row += "&nbsp;<a href='#' onclick='confirmdelete(\"" + item.Stringİd + "\")'>delete</a></td>";
    row += "</tr>";
    return row;
}
...
function showedit(id, isnew) {
    ...
    if (isnew)
    {
        $("#Stringİd").removeAttr('readonly');
        $("#formcaption").html("Add new item");
        $("#hiddenid").val(-1);
        $("#updatebtn").html("Add");
    }
    else {
        $("#Stringİd").attr('readonly');
