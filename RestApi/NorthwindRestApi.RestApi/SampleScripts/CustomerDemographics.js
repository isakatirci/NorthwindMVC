...
var uri = 'api/customerdemographics';
...
function updateitem() {
    $.ajax({
        type: 'Put',
        url: window.getBaseUrl() + uri + "/" + $('#CustomerTypeID').val(),
        data: getformvalues(),
        dataType: "json"
    })
   .done(function ()
...
function formatItem(item) {
    ...
    row += "<td><a href='#' onclick='showedit(\"" + item.CustomerTypeID + "\", false)'>edit</a>";
    row += "&nbsp;<a href='#' onclick='confirmdelete(\"" + item.CustomerTypeID + "\")'>delete</a></td>";
    row += "</tr>";
    return row;
}
...
function showedit(id, isnew) {
    ...
    if (isnew)
    {
        $("#CustomerTypeID").removeAttr('readonly');
        $("#formcaption").html("Add new item");
        $("#hiddenid").val(-1);
        $("#updatebtn").html("Add");
    }
    else {
        $("#CustomerTypeID").attr('readonly');
